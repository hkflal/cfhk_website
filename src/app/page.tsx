        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-12 items-start"> {/* Changed gap-8 to gap-12, items-center to items-start */}
            <div className="bg-neutral-50 p-8 rounded-xl shadow-xl"> {/* Added bg, padding, shadow for consistency */}
              <h2 className="text-3xl sm:text-4xl font-bold text-primary-600 mb-6 tracking-tight font-inter">聯絡我們</h2> {/* Adjusted heading style */}
              <p className="mb-6 text-neutral-600 leading-relaxed">如果您對我們的服務有任何疑問，或想了解更多關於香港外勞政策的資訊，歡迎隨時聯絡我們。</p>
              
              <div className="mb-8 space-y-4"> {/* Increased spacing */}
                <div className="flex items-start"> {/* items-start for better alignment with multi-line text */}
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-primary-500 mr-3 mt-1 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor"> {/* Adjusted icon size and color */}
            ...
            <div className="bg-neutral-50 p-8 rounded-xl shadow-xl"> {/* Added bg, padding, shadow for consistency */}
              {contactFormSubmitted ? (
                <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center h-full flex flex-col justify-center"> {/* Ensure it fills height */}
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-green-500 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"> {/* Increased icon size */}
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <h2 className="text-2xl font-bold text-green-800 mb-2">感謝您的查詢</h2> {/* Increased font size */}
                  <p className="text-green-700 text-lg">我們已收到您的留言，將盡快與您聯絡。</p> {/* Increased font size */}
                  <p className="text-sm text-neutral-500 mt-4">正在返回首頁...</p>
                </div>
              ) : (
                <form className="space-y-6" onSubmit={handleContactSubmit}> {/* Increased spacing */}
                  <h2 className="text-2xl sm:text-3xl font-bold text-neutral-800 mb-6 tracking-tight font-inter">發送即時查詢</h2> {/* Adjusted heading */}
                  <div>
                  ...
                  <button 
                    type="submit" 
                    className="w-full bg-primary-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-primary-700 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 shadow-md"
                  >
                    立即提交
                  </button>
                </form>
